...
local function method_handler(method, stream)
	print(method)
	if method == "POST" then
		local body = assert(stream:get_body_as_string(100))
		print(body)
...
local function reply(myserver, stream) -- luacheck: ignore 212
	...
	local response = method_handler(req_method, stream)
